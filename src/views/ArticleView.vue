    ...
    <div class="row">
      <div v-for="x in list" :key="x.id" class="col-sm-4 col-12 mb-3">
        <div class="card rounded-4 p-3 border-0 shadow">
            <img
              :src="x.pic_link"
              class="card-img-top ar-card rounded-4"
              alt="..."
            />
          <div class="card-body">
            ...
            <router-link
              :to="{ name: 'article-details', params: { id: x.id,slug:x.slug } }"
              href="#"
              class="text-primary text-decoration-none"
              >مطالعه مقاله >></router-link
            >
          </div>
        </div>
      </div>
  ...
  <div class="w-100 mb-5 mt-5">
    <nav aria-label="Page navigation example">
      <ul class="pagination justify-content-center">
        ...
        <li
          v-for="index in page_number"
          :key="index"
          v-bind:class="page == index ? 'active' : ''"
          class="page-item"
      ...
      axios
        .get(
          import.meta.env.VITE_API_DOMAIN +
            "api/v1/article/?page_size=9&page=" +
            this.page
        )
        .then((response) => {
